...
Audio Dataset Voiceprint Recognition by 2DIPW based on yyupiaoling/VoiceprintRecognition-Pytorch
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lp', '--label_path', type=str, default="labels/", help="Path to Voice feature library.")
    parser.add_argument('-t', '--threshold', type=float, default=0.6, help="Threshold for judging compliance.")
    parser.add_argument('-mp', '--model_path', type=str, default="model/", help="Path to model.")
    ...
    for i, file in enumerate(input_files):
        ...
        if label:
            print(
                f"\033[32m[{i}/{input_files_amount}]\033[0m \033[33m{os.path.basename(file)}\033[0m is recognized as speaker \033[31m{label}\033[0m, the max similarity is \033[34m{similarity}\033[0m.")
            result_dicts_list.append(
                {"filepath": file, "label": labels_dict[label], "similarity": float(similarity)})
        else:
            print(
                f"\033[32m[{i}/{input_files_amount}]\033[0m \033[33m{os.path.basename(file)}\033[0m could not be recognized as any speaker.")
            result_dicts_list.append(
                {"filepath": file, "label": None, "similarity": None})
    ...
    json_path = os.path.join(output_path_for_this_run, "result.json")

    # Move input files to category folders
    print("Moving input files to category folders...")
    folder_list = [os.path.join(output_path_for_this_run, "Unrecognized")]
    folder_list += [os.path.join(output_path_for_this_run, label) for label in labels_list]
    for folder in folder_list:
        if_not_mkdir(folder)
    for result in result_dicts_list:
        destination_folder = folder_list[result["label"]] if result["label"] else folder_list[0]
        try:
    ...
    with open(json_path, "w") as f:
        json.dump({"labels": labels_dict, "files": result_dicts_list}, f, indent=4)
